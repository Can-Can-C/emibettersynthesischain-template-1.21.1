...
│     ├─ Config.java                     ModConfigSpec 配置值（SPEC 注册在平台入口）
│     ├─ client/                         同 7a 的 client/* 全部
│     └─ mixin/                          同 7a 的 mixin/* 全部
└─ neoforge/                          平台模块（main/26.1.2；1.20.1 分支为 forge/）
   └─ src/main/
      ├─ java/com/cancan/emibettersynthesischain/
      │  ├─ EMIBettersynthesischainMod.java   @Mod（BOTH：注册 Config.SPEC + commonSetup）
      │  └─ EMIBettersynthesischainClient.java @Mod(CLIENT)：事件总线/快捷键/滚轮/屏幕切换
      └─ resources/                    全部资源（assets lang、mixins.json、模板 mods.toml、datagen 输出）
```
