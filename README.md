...
2. **合成树布局**：最终产物列可置**左或右**（设置 `treeGoalSide`，默认右），材料区按行横排（最深→最浅，最后一行=目标直接输入），副产物在底部；**纯图标 + 右下角数量（大数字缩写 k/M/G/T）**，顶部总材料行（种类过多自动换行铺满，左侧标 **S(sum)**），材料行左侧**层级序号 1、2、…**（自上而下），每棵树左侧"["**括号**括住整棵配方；材料不足时**左上角红字显示拥有量**；悬停显示物品栏样式 tooltip + **合成明细**（需要/拥有/由 X 合成/可合成、不足红字）。
...
| `treeLineColor` | teal | 线条配色预设（连接线/树括号/层级序号/S 标记）：青绿/蓝/紫/白/金/红/绿/灰 |
